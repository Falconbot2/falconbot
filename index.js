...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:Affis Admin\n' 
            + 'ORG: Pengembang XBot;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=5521993471668:+55 21 99347-1668\n' 
            + 'END:VCARD' 
prefix = '!'
blocked = []          
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `olá @${num.split('@')[0]}\ bem vindo ao *${mdata.subject}* / espero que goste do grupo😅😅`
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["5521993471668@s.whatsapp.net"] 
			...
			switch(command) {
				...
				case 'info':
					...
					break
				case 'listabloq': 
					teks = 'dei block nesses bobos aí🤣'
...
case 'lofi':
                ...
                case 'reportarbug':
                    ...
                    client.sendMessage('5521993471668@s.whatsapp.net', options, text, {quoted: mek})
            ...
            case 'criador':
            case 'FALCON':
                  client.sendMessage(from, {displayname: "FALCON", vcard: vcard}, MessageType.contact, { quoted: mek})
           ...
           case 'rebaixar':
			...
			case 'remover':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `removi mermo hahaha :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupRemove(from, mentioned)
					} else {
						mentions(`removi mermo @${mentioned[0].split('@')[0]}  hahaha`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
				...
				case 'adms':
					if (!isGroup) return reply(mess.only.group)
					teks = `adms do grupo *$Metadata.subject}*\nadms : ${groupAdmins.length}\n\n`
					no = 0
...
case 'simih':
 ...
 responder ('Sukses mengaktifkan mode simi di group ini 鉁旓笍')
 } else if (Number (args [0]) === 0) {
 samih.splice (de, 1)
 fs.writeFileSync ('./ src / simi.json', JSON.stringify (samih))
 responder ('Sukes menonaktifkan mode simi di group ini 鉁旓笍')
 } outro {
 ...
 intervalo
               ...
               if (isBanned) return reply(ind.benned(checkMoti))
               if (isLimit(sender)) return reply(ind.limitend(pushname))
               if (!isRegistered) return reply(ind.noregis(prefix))
